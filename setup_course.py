...
def main():
    ...
    try:
        with open(cred_file, 'w', encoding='utf-8') as f:
            ...
            f.write("import os\n")
            f.write("import sys\n\n")
            f.write("# Auto-restart in virtual environment if needed\n")
            f.write("def _ensure_venv():\n")
            f.write("    root_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))\n")
            f.write("    venv_python = os.path.join(root_dir, 'venv', 'bin', 'python')\n")
            f.write("    if os.name == 'nt':\n")
            f.write("        venv_python = os.path.join(root_dir, 'venv', 'Scripts', 'python.exe')\n")
            f.write("    if os.path.exists(venv_python) and os.path.abspath(sys.executable) != os.path.abspath(venv_python):\n")
            f.write("        os.execv(venv_python, [venv_python] + sys.argv)\n")
            f.write("_ensure_venv()\n\n")
